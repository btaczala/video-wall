...
SDLTexture::SDLTexture(SDL_Renderer* renderer, std::uint16_t width,
                       std::uint16_t height)
    : _renderer(renderer),
      _texture(SDL_CreateTexture(_renderer, SDL_PIXELFORMAT_UNKNOWN,
...
void SDLTexture::put(const void* buffer, const TextureSize& size) noexcept {
    ...
    int texture_pitch = 0;
    SDL_LockTexture(_texture.get(), 0, reinterpret_cast<void**>(&texture_data),
                    &texture_pitch);
...
std::unique_ptr<ITexture> SDLRenderer::createTexture(std::uint16_t w,
